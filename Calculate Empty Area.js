...
function find_angle(Ax,Ay,Bx,By,Cx,Cy) {
   ...
   if (c.x > x0 && c.x<x1 && c.y < y0 && c.y1 > y0 && c.y1 < y1){
    ...
    if (c.x > x0 && c.x<x1 && c.y > y1 && c.y0 < y1 && c.y0>y0){
    ...
    return;
      ...
      if (c.x < x0 && c.x1>x0 && c.y > y0 && c.y < y1 && c.x1<x1){
    ...
    return;
     ...
     var angle= find_angle(c.x-AX,y0,c.x,c.y,c.x+AX,y0);
     res.json(s-(area_traingle-Math.round((Math.PI * c.r * c.r*angle/(2*Math.PI)) * 100) / 100));
    return;
  }
